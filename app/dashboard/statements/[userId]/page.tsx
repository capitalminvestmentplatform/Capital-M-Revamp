...
export default function UserStatementsPage() {
  ...
  const fetchUser = async (
    ...
    try {
      ...
      const { items, pagination } = json.data as {
        items: StatementRow[];
        pagination: any;
      };
      setRows(items || []);
      ...
      const first = items?.[0];
      if (first) {
        if (!headerName && first.username) setHeaderName(first.username);
        if (!headerCode && first.clientCode) setHeaderCode(first.clientCode);
      } else {
